...
public class Perceptron
{
   ...
   private void readFile(String fileName) throws Exception
   {
      ...
      String[] bmpFileNames = br.readLine().split(" ");
      ...
      expectedValue = new double[numCases][2][]; //sets to however many test cases there are

      if (!bmpFileNames[0].contains(".bmp"))
      {
      ...
      {
         numCases = bmpFileNames.length;
         expectedValue = new double[numCases][2][];

         for (int testCase = 0; testCase < numCases; testCase++)
         {

            expectedValue[testCase][0] = DibDump.readPels(bmpFileNames[testCase]);
            expectedValue[testCase][1] = new double[outputLayerSize];

            for (int i = 0; i < outputLayerSize; i++)
            {
               if (testCase == i)
                  expectedValue[testCase][1][i] = 1;
               else
                  expectedValue[testCase][1][i] = 0;
            }
         }
   ...
   private double[] calculateOutput(double[] inputs)
   {
      ...
      for (int i = 0; i < numInputs; i++) // First initializes the first layer of the activation aka the input
      {
   ...
   private void optimizeWeight()
   {
      ...
      do {
         ...
         System.out.print("\r" + currentError);
      ...
      for (int test = 0; test < numCases; test++)
      {
         ...
         {
            activations[inputLayer][j] = expectedValue[test][0][j];
         }
         calculateOutput(expectedValue[test][0]);

         for (int i = 0; i < outputLayerSize; i++)
         {
            System.out.println("Test outputs for case " + i + ": " + expectedValue[test][1][i]);
            System.out.println("Real Outputs for case " + i + ": " + activations[outputLayer][i]);
         }
         System.out.println("\nError is " + errorFunction());
         currentError += errorFunction();
   ...
   /**
    * Stores the weights onto a different file to be read later
    *
    * @param fileName the name of the file being written to
    * @throws Exception if there is no file
    */
   public void storeWeights(String fileName) throws Exception
   {
   ...
   /**
    * Finds the weights in a file and adds them to the perceptron network
    *
    * @param fileName the name of the file containing the weights
   ...
   public void readWeights(String fileName) throws Exception
   {
      ...
      for (int n = 0; n < numLayers - 1; n++)
      {
         for (int k = 0; k < layers[n]; k++)
         {
            for (int j = 0; j < weights[n][k].length; j++)
            {
               weights[n][k][j] = Double.parseDouble(weightList[counter]);
               counter++;
            }
         }
      }
...
//      fileName = in.nextLine();
      ...
      p.storeWeights("Writable.txt");
      p.readWeights("Writable.txt");

      for (int i = 0; i < p.outputLayerSize; i++)
      {
         System.out.print(p.calculateOutput(p.expectedValue[2][0])[i] + ", ");
      }
